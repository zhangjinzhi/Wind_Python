...
def calculate_rating_change(table_name,stock_code,date,one_column,two_column,three_column,insert_column):
    db = MySQLdb.connect(host='127.0.0.1', user='root', passwd='zjz4818774', db='invest_latest', port=3306,charset='utf8')
    # 使用cursor()方法获取操作游标
    cursor = db.cursor()
    ...
    try:
        ...
        if row[0] == None or row[1] == None or row[2] == None or row[2] == 0:
            result = None
        else:
            result = float(row[0] -row[1])/row[2]
        print result
    except MySQLdb.Error, e:
        print "Mysql Error %d: %s" % (e.args[0], e.args[1])
        return ""
...
def calculate_store_rating_change(symbols,table_name,one_column,two_column,three_column,insert_column):
    db = MySQLdb.connect(host='127.0.0.1', user='root', passwd='zjz4818774', db='invest_latest', port=3306,charset='utf8')
    cursor = db.cursor()
    ...
    for stock_code in symbols:
        ...
        for date in date_list:
            calculate_rating_change(table_name,stock_code,date,one_column,two_column,three_column,insert_column)
##########################################################################################################################
##########################################################################################################################
##########################################################################################################################
def calculate_rating_diff_change(table_name,stock_code,date,one_column,two_column,insert_column):
    db = MySQLdb.connect(host='127.0.0.1', user='root', passwd='zjz4818774', db='invest_latest', port=3306,charset='utf8')
    # 使用cursor()方法获取操作游标
    cursor = db.cursor()
    # date = '2010-06-30 00:00:00'
...
def calculate_store_rating_diff_change(symbols,table_name,one_column,two_column,insert_column):
    db = MySQLdb.connect(host='127.0.0.1', user='root', passwd='zjz4818774', db='invest_latest', port=3306,charset='utf8')
    cursor = db.cursor()
    sql = "alter table "+table_name+" add "+insert_column+" double"
    try:
    ...
    for stock_code in symbols:
        date_list = get_date_from_table(table_name,stock_code)
        for date in date_list:
            calculate_rating_diff_change(table_name,stock_code,date,one_column,two_column,insert_column)
...
if __name__ == "__main__":
    ...
    one_column = "rating_upgrade"
    two_column = "rating_downgrade"
    three_column = "rating_instnum"
    insert_column = "rating_change"
    calculate_store_rating_change(symbols,table_name,one_column,two_column,three_column,insert_column)
    #################################################################################
    table_name = "table_month_data"

    one_column = "wrating_targetprice"
    two_column = "close"
    insert_column = "rating_diff_change"
    calculate_store_rating_diff_change(symbols,table_name,one_column,two_column,insert_column)
    ################################################################################
