...
def calculate_ratio_function(table_name,stock_code,date,one_column,two_column,three_column,four_column,insert_column):
    db = MySQLdb.connect(host='127.0.0.1', user='root', passwd='zjz4818774', db='invest', port=3306,charset='utf8')
    # 使用cursor()方法获取操作游标
    cursor = db.cursor()
    # date = '2010-06-30 00:00:00'
    sql =  "SELECT "+one_column+","+two_column+","+three_column+","+four_column+" FROM "+table_name+" WHERE stock_code='"+stock_code+"' AND trade_date = '"+date+"'"
    ...
    try:
        ...
        if row[0] == None or row[1] == None or row[2] == None:
            ...
            temp = row[0]+row[1]
            longdebt = row[2]*row[3]

            result = (temp+longdebt)/row[0]
        print result
    except MySQLdb.Error, e:
        print "Mysql Error %d: %s" % (e.args[0], e.args[1])
        return ""
...
def calculate_store_ratio(symbols,table_name,one_column,two_column,three_column,four_column,insert_column):
    db = MySQLdb.connect(host='127.0.0.1', user='root', passwd='zjz4818774', db='invest', port=3306,charset='utf8')
    cursor = db.cursor()
    ...
    for stock_code in symbols:
        ...
        for date in date_list:
            calculate_ratio_function(table_name,stock_code,date,one_column,two_column,three_column,four_column,insert_column)
...
##########################暂时不用这个指标########################
    ...
    two_column = "share_ntrf_prfstock"
    three_column = "longdebttodebt"
    four_column = "tot_liab"
    insert_column = "mc_lev"
    calculate_store_ratio(symbols,table_name,one_column,two_column,three_column,four_column,insert_column)
