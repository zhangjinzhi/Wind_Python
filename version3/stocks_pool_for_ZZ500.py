...
def get_stockcode_from_table(table_name):
    ...
    sql = "SELECT ZZ500_code FROM "+table_name+" ORDER BY ZZ500_code ASC"
    try:
        cursor.execute(sql)
...
def get_zz500_stocks_pool():
    ...
    #取沪深300指数中股票代码和权重
    ZZ500_stocks = w.wset("IndexConstituent","date=20170601;windcode=000905.SH;field=wind_code,sec_name,i_weight")
    #取停牌信息
    # w.wset("TradeSuspend","startdate=20150101;enddate=20130608;field=wind_code,sec_name,suspend_type,suspend_reason")
    print ZZ500_stocks
    ZZ500_stocks_data = pd.DataFrame()
    ZZ500_stocks_data['ZZ500_code']=ZZ500_stocks.Data[0]
    ZZ500_stocks_data['sec_name']=ZZ500_stocks.Data[1]
    ZZ500_stocks_data['i_weight']=ZZ500_stocks.Data[2]
    ZZ500_table_name = 'zz500_stocks'
    try:
        ZZ500_stocks_data.to_sql(ZZ500_table_name,engine,if_exists='replace')
    except Exception as e:
        #如果写入数据库失败，写入日志表，便于后续分析处理
        error_log=pd.DataFrame()
        error_log['table']=ZZ500_table_name
...
def store_ipo_date_into_table(table_name,stock_code):
    ...
    sql = "UPDATE "+table_name+" SET ipo_date='"+ipo_date+"' WHERE ZZ500_code='"+stock_code+"'"
    # print sql
    try:
        cursor.execute(sql)
    except MySQLdb.Error, e:
...
def get_stockcode_secname_ipodate_from_table(table_name):
    ...
    sql = "SELECT ZZ500_code,sec_name,ipo_date FROM "+table_name+" ORDER BY ZZ500_code ASC"
    try:
        cursor.execute(sql)
        rows = cursor.fetchall()
        for row in rows: 
...
def zz500_stocks_ipo_date_delete(table_name):
...
def zz500_stocks_ST_delete(table_name):
    table_name = "zz500_stocks"
    stockcode_list,secname_list,ipodate_list = get_stockcode_secname_ipodate_from_table(table_name)
...
def select_good_stocks():
    ...
    stocks_ST_delete_list = zz500_stocks_ST_delete(table_name)
    print stocks_ST_delete_list
    print "len(stocks_ST_delete_list) is :",len(stocks_ST_delete_list)

    stocks_IPO_Date_delete_list = zz500_stocks_ipo_date_delete(table_name)
...
if __name__ == "__main__":
    ...
    # get_zz500_stocks_pool()
    # 
    # store_ipodate_column_into_table(table_name="zz500_stocks")
    # 
    # ipo_date = "2007-12-31 00:00:00"
    # given_date= "2007-12-31 00:00:00"
    ...
    # zz500_stocks_ipo_date_delete(table_name = "zz500_stocks")
    # 
    # 
    # 
    # 
