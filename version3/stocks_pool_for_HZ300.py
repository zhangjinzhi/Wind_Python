...
def get_hz300_stocks_pool():
    ...
    engine = create_engine('mysql://root:zjz4818774@localhost/invest_lastest?charset=utf8')
    # print engine
    w.start()
...
def get_industry(stock_code):
    w.start()
    result = w.wsd(stock_code, "industry_citic,industry_citiccode", "2017-06-01", "2017-06-01", "industryType=1;Period=M;Days=Alldays;Fill=Previous;PriceAdj=F")
    print result.Data
    print result.Data[0][0]

    industry_citic = result.Data[0][0]
    industry_citiccode = result.Data[1][0]
    return str(industry_citic),str(industry_citiccode)

def add_industry_column(table_name):
    db = MySQLdb.connect(host='127.0.0.1', user='root', passwd='zjz4818774', db='invest_lastest', port=3306,charset='utf8')
    cursor = db.cursor()
    sql = "alter table "+table_name+" add industry_citic text"
    try:
        cursor.execute(sql)
        # print "add key:ipo_date successfully"
    except MySQLdb.Error, e:
    ...
    sql = "alter table "+table_name+" add industry_citiccode text"
    try:
        cursor.execute(sql)
        # print "add key:ipo_date successfully"
    except MySQLdb.Error, e:
...
def store_industry_into_table(table_name,stock_code):
    db = MySQLdb.connect(host='127.0.0.1', user='root', passwd='zjz4818774', db='invest_lastest', port=3306,charset='utf8')
    cursor = db.cursor()

    industry_citic,industry_citiccode = get_industry(stock_code)


    sql1 = "UPDATE "+table_name+" SET industry_citic='"+industry_citic+"' WHERE hz300_code='"+stock_code+"'"
    # print sql
    try:
        cursor.execute(sql1)
    except MySQLdb.Error, e:
    ...
    sql2 = "UPDATE "+table_name+" SET industry_citiccode='"+industry_citiccode+"' WHERE hz300_code='"+stock_code+"'"
    # print sql
    try:
        cursor.execute(sql2)
    except MySQLdb.Error, e:
...
def store_industry_column_into_table(table_name):
    #添加两列add_industry_column到数据表中
    add_industry_column(table_name)
    #获取股票代码
    stockcode_list = get_stockcode_from_table(table_name)
    # 逐个获取ipo_date,然后存入数据表中
    for stock_code in stockcode_list:

        store_industry_into_table(table_name,stock_code)
...
def hz300_stocks_ipo_date_delete(table_name):
    ...
    # 
    given_date= "2016-12-31 00:00:00"
...
if __name__ == "__main__":
    ...
    store_industry_column_into_table(table_name="hz300_stocks")
