...
def get_factor_MonthReturn_from_table(factor,MonthReturn,table_name,trade_date):
    ...
    sql = "select "+factor+","+MonthReturn+" from "+table_name+" where trade_date='"+trade_date+"' ORDER BY stock_code ASC"
...
def get_dummy(trade_date):
    ...
    dummy_df = add_dummy_avariable(table_name="all_stocks_cleaned_factors_table")
...
def output_tvalues_correlation(x_list,y_list,trade_date):

    dummy = get_dummy(trade_date)
    print "len(dummy) is : ",len(dummy)
    print dummy
    print "len(dummy[:,1:]) is : ",len(dummy[:,1:])
    print dummy[:,1:]
    print "len(x_list) is : ",len(x_list)
    print x_list
    ...
    print 'results.summary() is : '
    print results.summary()
    print 'results.params is : ' 
    print results.params

    # print 't: ', results.tvalues[1]
    # 相关性系数，加了常数项
    correlation = results.params[0]
...
def store_tvalues_correlation(outputFp,x_list,y_list,trade_date,factor):
    ...
    tvalues,correlation = output_tvalues_correlation(x_list,y_list,trade_date)
...
if __name__ == "__main__":
    ...
    # x_list,y_list = get_factor_MonthReturn_from_table(factor,MonthReturn,table_name,trade_date)
    ######################################################################################
    # x_list,y_list = [1,2,100,4,5] , [10,9,8,7,6]
