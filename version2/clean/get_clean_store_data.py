...
def pandas_read_data_from_table(stock_code,date):
    ...
    conn = pymysql.connect(host='127.0.0.1', port=3306, user='root', passwd='zjz4818774', db='invest_after_calculation')
    cursor = conn.cursor()
...
def store_modified_data_into_new_table(clean_data_df):
    ...
    engine = create_engine('mysql://root:zjz4818774@localhost/invest_after_calculation?charset=utf8')
    try:
